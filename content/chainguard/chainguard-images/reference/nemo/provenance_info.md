---
title: "Provenance Information for nemo Images"
type: "article"
unlisted: true
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/nemo/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for nemo Image
...
## Verifying nemo Image Signatures
The **nemo** Chainguard Images are signed using Sigstore, and you can check the included signatures using `cosign`.
...
cgr.dev/chainguard-private/nemo | jq
```

## Downloading nemo Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the nemo image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the nemo image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/nemo | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/nemo | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying nemo Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the nemo image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/nemo
```

This will pull in the signature for the attestation specified by the `--type` parameter, which in this case is the SPDX attestation. You will receive output that verifies the SBOM attestation signature in cosign's transparency log:

```
Verification for cgr.dev/chainguard/nemo --
