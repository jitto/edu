---
title: "Provenance Information for loki Images"
type: "article"
unlisted: true
description: "Provenance information for loki Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/loki/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for loki Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                                                   |
|------------------------------|--------------------------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | latest-dev                                                                                             |
| `cgr.dev/chainguard-private` | 2, 2-dev, 2.9, 2.9-dev, 2.9.8, 2.9.8-dev, 3, 3-dev, 3.0, 3.0-dev, 3.0.0, 3.0.0-dev, latest, latest-dev |
...
cgr.dev/chainguard-private/loki | jq
```
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the loki image on `linux/amd64`:
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/loki | jq -r .payload | base64 -d | jq .predicate
```
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/loki
```
