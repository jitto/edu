---
title: "Provenance Information for r-base Images"
type: "article"
unlisted: true
description: "Provenance information for r-base Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/r-base/provenance_info/" >}}
{{</ tabs >}}
...
| `cgr.dev/chainguard-private` | 4-dev, 4.3-dev, 4.3.1-dev, latest-dev |
...
The **r-base** Chainguard Images are signed using Sigstore, and you can check the included signatures using `cosign`.
...
cgr.dev/chainguard-private/r-base | jq
```

## Downloading r-base Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the r-base image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the r-base image on `linux/amd64`:
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/r-base | jq -r .payload | base64 -d | jq .predicate
```
...
You can use the `cosign verify-attestation` command to check the signatures of the r-base image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/r-base
```
